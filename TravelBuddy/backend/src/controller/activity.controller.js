...
export const updateActivity = asyncHandler(async (req, res, next) => {
   try {
      ...
      activity.location.coordinates = [
         req.body.lng || activity.location.coordinates[0],
         req.body.lat || activity.location.coordinates[1],
      ];
...
export const getSingleActivity = asyncHandler(async (req, res, next) => {
   try {
      const activityId = req.params.id;
      const activity = await Activity.findById(activityId).populate("creator", {
         fullName: 1,
         profilePicture: 1,
      });
      if (!activity) {
...
export const MyCreatedActivites = asyncHandler(async (req, res, next) => {
   ...
   const activities = await Activity.find({ creator: userId }).populate(
      "creator",
...
export const getParticipants = asyncHandler(async (req, res, next) => {
   ...
   const activity = await Activity.findById({ _id: activityId.id }).populate(
      "participants",
      "fullName profilePicture email mobile createdAt",
   );
   if (!activity) {
      return next(new ApiError(404, "Activity not found"));
   }
   ...
   }).select("fullName profilePicture email mobile createdAt");
   res.status(200).json({
      data: participants,
   });
});
