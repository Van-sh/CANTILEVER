...
export const UpdateActivity = createAsyncThunk(
   "user/UpdateActivity",
   async (
      { activityId, title, description, date, location, participantLimit, lat, lng },
      { rejectWithValue },
   ) => {
      try {
      ...
      builder
         ...
         .addCase(leaveActivity.fulfilled, (state, action) => {
            ...
            state.joinedActivities = state.joinedActivities.filter(
               (activity) => activity._id !== action.payload.data.activityId,
            );
         })
         ...
         .addCase(deleteActivity.fulfilled, (state, action) => {
            ...
            state.createdActivity = state.createdActivity.filter(
               (activity) => activity._id !== action.payload.data.activityId,
            );
         })
